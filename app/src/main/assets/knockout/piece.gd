...
const MAX_ARROW_LENGTH: float = 275.0
...
func _fade_out_arrow(fade_sec: float) -> void:
	...
	arrow_fade_tween.tween_property(arrow_root, "modulate:a", 0.0, fade_sec).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
...
func show_arrow_from_replay(angle_deg: float, pow_px: float, fade_sec: float = 0.18) -> void:
	...
	if arrow_root:
		...
		arrow_root.modulate.a = 0.0
		arrow_fade_tween = create_tween()
		arrow_fade_tween.tween_property(arrow_root, "modulate:a", 1.0, fade_sec).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
