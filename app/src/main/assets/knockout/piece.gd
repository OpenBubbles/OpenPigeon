...
# Values verified by live Frida instrumentation of iOS Knockout:
#   b2World:    dt=1/60, velIters=60, posIters=60, gravity=(0,0)
#   Penguin body: linearDamping=1.35, angularDamping=0, allowSleep=false,
#                 fixedRotation=false, bullet=true, gravityScale=1 (irrelevant)
#   Penguin fixture: friction=1.0, restitution=1.0, density=1.0,
#                    shape=circle r=12.5
#   Shoot:      body.linearVelocity = 2 * dist * (cos(angle), sin(angle))
#   Empirically confirmed damping ratio per frame at 60Hz:
#                 |v_next| / |v_now| = 1 / (1 + 1.35 * 1/60) = 0.9780
var phys := {
	"PPM": 32.0,                         # pixels per meter (info only here)
	"PIECE_RADIUS_PX": 12.5,            # collider radius matches iOS Box2D radius
	"FRICTION": 1.0,                    # iOS value
	"RESTITUTION": 1.0,                 # iOS value — fully elastic
	"LINEAR_DAMP": 1.35,                # iOS value
	"ANGULAR_DAMP": 0.0,                # iOS value — no spin damping
	"DENSITY": 1.0,                     # iOS value
	"GRAVITY_SCALE": 0.0,               # no gravity (top-down)
	"CCD_MODE": RigidBody2D.CCD_MODE_CAST_RAY,  # iOS uses bullet=true (CCD)
	"LOCK_ROTATION": false,
	"CAN_SLEEP": false              # iOS fixedRotation=false
}
...
# ---------- Firing ----------
# Direct port of -[KnockScene ShootShoot] at 0x100014c40+.
# Frida-verified on iOS Knockout 2026-05-15:
#   PRE-SHOT:  vel=(0.000, 0.000)
#   POST-SHOT: vel=(193.525, -28.062), |v|=195.549, body.angle=1.427 rad
#   arrow.zRotation = atan2(-28.062, 193.525) = -0.1444 rad
#   dist = |v|/2 = 97.77, so v = 2 * dist * (cos(angle), sin(angle))  ✓
#   And body.angle = arrow.zRotation + π/2 (iOS rotates sprite to "face" launch).
# Also sets bullet=true (CCD) behavior already on via CCD_MODE.
func fire_from_current_aim() -> void:
	if power <= 0.5:
		return
	sleeping = false
	# Snap body rotation to face the shot (iOS does this via b2Body::SetTransform
	# before setting velocity). Our Godot RigidBody2D rotation is the same as
	# Box2D body.angle in radians. aim_dir.angle() returns the arrow angle.
	angular_velocity = 0.0
	rotation = aim_dir.angle() - PI * 0.5
	linear_velocity = aim_dir * (power * 2.0)
