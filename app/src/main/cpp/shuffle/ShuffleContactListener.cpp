...
namespace {
    ...
    bool isBumper(
        ...
        return data &&
               data->type ==
               ShuffleData::Type::Bumper;
    }

    bool isPuckInvolved(
            const ShuffleData* dataA,
            const ShuffleData* dataB
    ) {
        return isPuck(dataA) ||
               isPuck(dataB);
    }

    ShufflePuck* puckForData(
            ShuffleData* data
    ) {
        if (!isPuck(data)) {
            return nullptr;
...
void ShuffleContactListener::setTraceContext(
        const char* runId,
        int shotIndex,
        int frame,
        const char* phase
...
void ShuffleContactListener::BeginContact(
        b2Contact* contact
) {
    if (!contact) {
        return;
    ...
    const b2Vec2 positionA =
            bodyA
            ? bodyA->GetPosition()
            : b2Vec2_zero;

    const b2Vec2 positionB =
            bodyB
            ? bodyB->GetPosition()
            : b2Vec2_zero;

    const b2Vec2 velocityBeforeA =
            bodyA
            ? bodyA->GetLinearVelocity()
            : b2Vec2_zero;

    const b2Vec2 velocityBeforeB =
            bodyB
    ...
    const bool aPuckBBumper =
            isPuck(dataA) &&
            isBumper(dataB);

    const bool aBumperBPuck =
            isBumper(dataA) &&
            isPuck(dataB);

    bool usedBumperRewrite =
            false;
    ...
    if (
        ...
        if (
            ...
            if (
                    awayFromBumper.LengthSquared() <=
                    0.000001f
                    ) {
                awayFromBumper =
                        puck->body->GetPosition() -
                        bumperBody->GetPosition();
    ...
    const b2Vec2 velocityAfterA =
            bodyA
            ? bodyA->GetLinearVelocity()
            : b2Vec2_zero;

    const b2Vec2 velocityAfterB =
            bodyB
            ? bodyB->GetLinearVelocity()
            : b2Vec2_zero;
    ...
    const float frictionA =
            fixtureA
            ? fixtureA->GetFriction()
            : 0.0f;

    const float frictionB =
            fixtureB
...
void ShuffleContactListener::PreSolve(
    ...
    const bool aPuckBWall =
            isPuck(dataA) &&
            isWall(dataB);

    const bool aWallBPuck =
            isWall(dataA) &&
            isPuck(dataB);
    ...
    ShuffleData* puckData =
            aPuckBWall
            ? dataA
            : dataB;

    ShuffleData* wallData =
            aPuckBWall
            ? dataB
            : dataA;
    ...
    const b2Vec2 velocity =
            puck->body->GetLinearVelocity();

    shuffleNativeLogPrint(
            ANDROID_LOG_INFO,
            "ShuffleNative",
            "SHUFFLE_NATIVE_GATE_FILTER={"
            "\"runId\":\"%s\","
            "\"shotIndex\":%d,"
            "\"frame\":%d,"
            "\"phase\":\"%s\","
...
void ShuffleContactListener::PostSolve(
    ...
    const b2Vec2 velocityA =
            bodyA
            ? bodyA->GetLinearVelocity()
            : b2Vec2_zero;

    const b2Vec2 velocityB =
            bodyB
            ? bodyB->GetLinearVelocity()
            : b2Vec2_zero;

    const float angularVelocityA =
            bodyA
            ? bodyA->GetAngularVelocity()
            : 0.0f;

    const float angularVelocityB =
            bodyB
